...
typedef enum { BORDER = 0,
               BASE,
               TEXT,
               OTHER } GuiPropertyElement;
...
void GuiEnable(void) {
    if (guiState == GUI_STATE_DISABLED) guiState = GUI_STATE_NORMAL;
}

// Disable gui global state
// NOTE: We check for GUI_STATE_NORMAL to avoid messing custom global state setups
void GuiDisable(void) {
    if (guiState == GUI_STATE_NORMAL) guiState = GUI_STATE_DISABLED;
}
...
#define RAYGUI_GROUPBOX_LINE_THICK 1
    ...
    // Draw control
    //--------------------------------------------------------------------
    GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle){bounds.x, bounds.y, RAYGUI_GROUPBOX_LINE_THICK, bounds.height}, 1, BLACK, Fade(GetColor(GuiGetStyle(DEFAULT, (state == GUI_STATE_DISABLED) ? BORDER_COLOR_DISABLED : LINE_COLOR)), guiAlpha));
    GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle){bounds.x, bounds.y + bounds.height - 1, bounds.width, RAYGUI_GROUPBOX_LINE_THICK}, 1, BLACK, Fade(GetColor(GuiGetStyle(DEFAULT, (state == GUI_STATE_DISABLED) ? BORDER_COLOR_DISABLED : LINE_COLOR)), guiAlpha));
    GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle){bounds.x + bounds.width - 1, bounds.y, RAYGUI_GROUPBOX_LINE_THICK, bounds.height}, 1, BLACK, Fade(GetColor(GuiGetStyle(DEFAULT, (state == GUI_STATE_DISABLED) ? BORDER_COLOR_DISABLED : LINE_COLOR)), guiAlpha));
...
#define RAYGUI_LINE_TEXT_PADDING 4
    ...
    //--------------------------------------------------------------------
    if (text == NULL)
        GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle){bounds.x, bounds.y + bounds.height / 2, bounds.width, 1}, 0, BLANK, color);
    else {
        ...
        GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle){bounds.x, bounds.y + bounds.height / 2, RAYGUI_LINE_MARGIN_TEXT - RAYGUI_LINE_TEXT_PADDING, 1}, 0, BLANK, BLACK);
        GuiDrawText(text, textBounds, GUI_TEXT_ALIGN_LEFT, BLACK);
        GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle){bounds.x + 12 + textBounds.width + 4, bounds.y + bounds.height / 2, bounds.width - textBounds.width - RAYGUI_LINE_MARGIN_TEXT - RAYGUI_LINE_TEXT_PADDING, 1}, 0, BLANK, BLACK);
    }
    //--------------------------------------------------------------------
}
...
Rectangle GuiScrollPanel(Rectangle bounds, const char *text, Rectangle content, Vector2 *scroll) {
    ...
    if ((state != GUI_STATE_DISABLED) && !guiLocked) {
        ...
        if (CheckCollisionPointRec(mousePoint, bounds)) {
            ...
            if (hasHorizontalScrollBar && (IsKeyDown(KEY_LEFT_CONTROL) || IsKeyDown(KEY_RIGHT_SHIFT)))
            ...
            if (textLength > 0) {
                if (IsKeyPressed(KEY_BACKSPACE)) {
                    if ((unsigned char)text[textLength - 1] < 127) {
                        ...
                        int charUTF8Length = 0;
                        while (((unsigned char)text[textLength - 1 - charUTF8Length] & 0b01000000) == 0)
                            charUTF8Length++;
...
float GuiSliderPro(Rectangle bounds, const char *textLeft, const char *textRight, float value, float minValue, float maxValue, int sliderWidth) {
    ...
    if ((state != GUI_STATE_DISABLED) && !guiLocked) {
        ...
        if (CheckCollisionPointRec(mousePoint, (Rectangle){bounds.x - 15, bounds.y - 15, bounds.width + 30, bounds.height + 30})) {
            if (IsMouseButtonDown(MOUSE_LEFT_BUTTON)) {
                state = GUI_STATE_PRESSED;
    ...
    if (textLeft != NULL) {
        ...
        textBounds.height = (float)GuiGetStyle(DEFAULT, TEXT_SIZE);
        textBounds.x = bounds.x - textBounds.width - GuiGetStyle(SLIDER, TEXT_PADDING) - 5;
        textBounds.y = bounds.y + bounds.height / 2 - GuiGetStyle(DEFAULT, TEXT_SIZE) / 2;
    ...
    if (textRight != NULL) {
        ...
        textBounds.height = (float)GuiGetStyle(DEFAULT, TEXT_SIZE);
        textBounds.x = bounds.x + bounds.width + GuiGetStyle(SLIDER, TEXT_PADDING) + 10;
        textBounds.y = bounds.y + bounds.height / 2 - GuiGetStyle(DEFAULT, TEXT_SIZE) / 2;
...
float GuiColorBarHue(Rectangle bounds, const char *text, float hue) {
    ...
    if (state != GUI_STATE_DISABLED) {
        ...
        DrawRectangleGradientV((int)bounds.x, (int)bounds.y, (int)bounds.width, (int)bounds.height, Fade(Fade(GetColor(GuiGetStyle(COLORPICKER, BASE_COLOR_DISABLED)), 0.1f), guiAlpha), Fade(GetColor(GuiGetStyle(COLORPICKER, BORDER_COLOR_DISABLED)), guiAlpha));
...
static Rectangle GetTextBounds(int control, Rectangle bounds) {
    ...
    switch (control) {
    case COMBOBOX:
        bounds.width -= (GuiGetStyle(control, COMBO_BUTTON_WIDTH) + GuiGetStyle(control, COMBO_BUTTON_SPACING));
        break;
    case VALUEBOX:
